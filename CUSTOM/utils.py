...
def visualize_heatmap(experiments, images, image_names, attmaps, epoch,cnt, phase='train', seg=False):
    ...
    for i in range(images.shape[0]):
        attmap = attmaps[i]
        img_name = image_names[i].split(".")[0]
        ...
        cv2.imwrite('./experiments/images/{}/{}/colormaps/{}.jpg'.format(experiments, phase, img_name), image)
        cv2.imwrite('./experiments/images/{}/{}/colormaps/{}-colormap.jpg'.format(experiments, phase, img_name), cam)
        ...
        if seg:
            ...
            cv2.imwrite('./experiments/images/{}/{}/colormaps/{}-seg.jpg'.format(experiments, phase, img_name), temp)
            mask = np.where(attmap>150,255,0)
            # cnt = iteration i = image number in the dataloader
            cv2.imwrite('./experiments/images/{}/{}/colormaps/{}-mask.jpg'.format(experiments, phase, img_name), mask)
