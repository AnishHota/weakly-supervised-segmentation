...
class ResNetSeries(nn.Module):
    def __init__(self, pretrained):
        ...
        if pretrained == 'supervised':
            ...
            model.load_state_dict(checkpoint['state_dict'], strict=False)
        # Added code for pretrained resnet using image level supervision
        elif pretrained == 'oxford':
            print(f'Loading unsupervised {pretrained} pretrained parameters!')
            model = resnet50(pretrained=False)
            model.fc = nn.Linear(model.fc.in_features,1)
            path_oxford_model = "D:\\Study\\Conda Projects\\opdl\\checkpoints\\model_oxf_resnet50_epochs10.pth"
            checkpoint = torch.load(path_oxford_model, map_location="cpu")
            model.load_state_dict(checkpoint, strict=False)
        elif pretrained=='polyp':
            print(f'Loading unsupervised {pretrained} pretrained parameters!')
            model = resnet50(pretrained=False)
            model.fc = nn.Linear(model.fc.in_features,1)
            path_oxford_model = "D:\\Study\\Conda Projects\\opdl\\checkpoints\\model_polyp_resnet50_epochs10.pth"
            checkpoint = torch.load(path_oxford_model, map_location="cpu")
            model.load_state_dict(checkpoint, strict=False)
        else:
